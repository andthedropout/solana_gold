...
- [DALL-E MCP Integration](#dall-e-mcp-integration)
...
This project includes optional integration with the [DALL-E MCP Server](https://github.com/garoth/dalle-mcp) for AI image generation during development. This allows you to generate images directly through AI conversations, which is particularly useful for creating hero images, feature graphics, and other visual content for your CMS sections.
...
3. **Enable the service by adding `dalle-mcp` to your profiles:**
   ```sh
   export COMPOSE_PROFILES=postgres,redis,assets,web,worker,dalle-mcp
   ```
...
The DALL-E MCP server will be available on port 3001 and images will be saved to `public/images/`.

**Note:** This integration is designed for local development only and requires an OpenAI API key. See [DALLE_MCP_INTEGRATION.md](DALLE_MCP_INTEGRATION.md) for detailed setup instructions, usage examples, and troubleshooting.
