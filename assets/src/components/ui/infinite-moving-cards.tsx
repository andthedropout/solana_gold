...
export const InfiniteMovingCards = ({
  ...
  function addAnimation() {
    if (containerRef.current && scrollerRef.current) {
      ...
      // Remove duplicated items if they exist
      while (scrollerRef.current.children.length > originalItemsCount) {
        scrollerRef.current.removeChild(scrollerRef.current.lastChild!);
      }

      // Add duplicated items for infinite scroll
      const currentItems = Array.from(scrollerRef.current.children);
  ...
  // Re-run animation when items change
  useEffect(() => {
    if (start) {
      setStart(false);
  ...
  // Re-run direction/speed updates when props change
  useEffect(() => {
    getDirection();
  }, [direction]);
